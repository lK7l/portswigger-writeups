...
9. Inject a UNION query that returns the DB version for MySQL/Microsoft (for example, using `SELECT @@version`) so it is reflected in the response.
10. Verify that the database version string is displayed on the web page.

## Result
The database version string is displayed on the web page.

## Why it works
The application uses the `category` input in a SQL query and reflects the results in the HTTP response, allowing a UNION-based injection to append additional rows to the original result set.
