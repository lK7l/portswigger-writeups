...
9. Inject a UNION query to retrieve username/password pairs concatenated into a single column: `' UNION SELECT NULL, username||'~'||password FROM users--`.
...
## Result
- Username and password pairs (concatenated with `~` separator) are displayed on the web page.
- Successful authentication as the `administrator` user.

## Why it works
The application uses the `category` input in a SQL query (for example: `SELECT name, description FROM products WHERE category = 'Accessories'`) and reflects the results in the HTTP response, allowing a UNION-based injection to append rows from the `users` table. When only one column can display text, string concatenation (`||` operator in PostgreSQL/Oracle/SQLite) allows retrieving multiple values (username and password) in a single column by combining them with a delimiter.
